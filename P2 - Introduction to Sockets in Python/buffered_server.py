...
class BufferedTCPEchoServer(object):
    ...
    def start(self):
        ...
        while self.keep_running:
            ...
            new_connection, addr = self.tcpServerSocket.accept()
            if new_connection:
                connected = True
                while connected and self.keep_running:
                    try:
                        data = new_connection.recv(4)
                        if data:
                            ...
                            while len(payloadBuffer) < length:
                                ...
                                data = new_connection.recv(bufferSize)
                                payloadBuffer += data
            
                            payload = payloadBuffer.decode()
                            newPayload = self.shorten(payload)
                            
                            new_connection.send(self.packMessage(newPayload))
                        else:
                            connected = False
